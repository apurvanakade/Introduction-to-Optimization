...
\Let $\bar{w}_1, \dots, \bar{w}_m$ be the basic variables, $\bar{x}_1, \dots, \bar{x}_n$ be the non-basic variables. Let $\bar{c}_j$, $\bar{b}_i$, and $\bar{a}_{ij}$ be the constants appearing in the dictionary at the BFS so that the dictionary is as follows:
\begin{equation}
...
::: {.example}
In the dictionary \@ref(eq:example-dictionary-1), the objective function is $4x + 3y$. Hence, both $x$ and $y$ can be chosen as entering variables. We can see that there are two different paths along the BFS going from the origin to the optimal solution. In the dictionary \@ref(eq:example-dictionary-2), the objective function is $3 + (-2)w_2 + y$ and only $y$ can be the entering variable. In dictionary \@ref(eq:example-dictionary-3), the objective function is $3.3 + (-5/3)w_2 + (-2/9)w_1$ and there cannot be any entering variable.
:::
...
When manipulating a linear system of equations, one can forget the variables and simply perform manipulations on the coefficients using matrices. 
...
If $\bar{x}_j$ is the entering variable and $\bar{w}_i$ is the leaving variable, then *we simply perform elementary row operations and turn the entry $\bar{a}_{ij}$ into a pivot for its column*. Hence, the simplex step is also called the **pivot step**.
