...
The basic feasible solutions are precisely the solutions obtained by changing $n$ or more inequalities to equations and solving the resulting simultaneous system of equations. As there are $n$ variables, we need to change at least $n$ equations in our simultaneous system of linear equations for the solution to be a single point. A degeneracy occurs when more than $n$ equations have a single solution, for example, when three lines in $\mathbb{R}^3$ or four planes in $\mathbb{R}^3$ intersecting at a single point.
...
::: {.exercise}
Find an upper bound on the set of basic feasible solutions for the standard linear program with $n$ decision variables and $m$ equations.
:::
