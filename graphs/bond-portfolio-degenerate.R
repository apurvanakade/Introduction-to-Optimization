...
bond_portfolio_degenerate <- ggplot() +
  xlim(0, max(b / A[, 1])) +
  ylim(0, max(b / A[, 2])) +
...
bond_portfolio_degenerate_solution <- bond_portfolio_degenerate +
  geom_point(aes(x=0.6, y=0.3), color = "black") +
  geom_text(aes(x=0.6, y=0.3), label="degenerate BFS", vjust=-.5, hjust=0) +
  stat_function(fun = function(x) {(4*0.6 + 3*0.3 - 4*x)/3}, color = "black")

print(bond_portfolio_degenerate_solution)
